...
def parse_args():
    ...
    parser.add_argument('--out_path', default="../../../results_val",help='path checkpoints')
...
def main():
    ...
    if not os.path.isdir(f"{args.out_path}/msk_pred/MedSAM_TEST"):
        os.mkdir(f"{args.out_path}/msk_pred/MedSAM_TEST")
    ...
    with torch.no_grad():
        for step, (image, gt2D, boxes, name) in enumerate(tqdm(nice_val_loader)):
            ...
            cv2.imwrite(f"{args.out_path}/msk_pred/MedSAM_TEST/{name[0]}.png",pred_img)

    pd.DataFrame(log).to_csv(f'{args.out_path}/csv/log_MedSAM_test.csv', index=False)
